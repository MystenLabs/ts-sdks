...
Both transports deliver similar end-to-end latency since network time dominates. The native gRPC
transport uses ~20% less CPU per request because it skips the base64 encoding layer that gRPC-web
text format requires, and avoids fetch API overhead.
