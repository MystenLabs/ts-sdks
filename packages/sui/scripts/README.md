...
writing files. It generates `tada-env.ts` using gql.tada's own introspection serializer without its
TypeScript-config loader, so generation works with the repository's TypeScript 7 toolchain.
