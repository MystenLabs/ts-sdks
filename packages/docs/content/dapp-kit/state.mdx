...
answer: >-
  DApp Kit manages wallet connection state, current account, and network
  selection through a centralized store that can be accessed via hooks (React)
  or direct subscription (core).
goal:
