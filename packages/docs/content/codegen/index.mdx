...
`configArguments` maps author-chosen keys to matchers, declared on the package entry whose generated
functions they configure. Because codegen output is network-agnostic, matchers never contain package
addresses: types are identified by `module::TypeName`, optionally qualified with a package
...
A bare `module::Type` refers to the declaring package's own type; types from other packages in the
run are referenced by their identifier and must be dependencies of the declaring package (anything
else is a generation-time error). Package entries must reference the declaring package itself.
Partially instantiated matchers (for example, `Pool<T>` or a nested uninstantiated generic) are not
supported. Use an uninstantiated matcher with a resolver function instead.

Misconfiguration is surfaced at generation time: malformed types, wrong arity, unknown package
identifiers, non-framework package addresses, references to non-dependency packages, and matched
types or functions missing from their package's summaries are all hard errors. Keys that never match
any generated parameter produce a warning. Note that the generated `config-arguments.ts` interface
covers all of the package's declared keys even when only some of its functions consume them; the
per-function `config` slices are the authoritative types for calls.
...
for arguments the caller did not pass:
...
Functions with matched parameters accept the named-object form of `arguments` (the tuple form is
positional for summaries without parameter names). If a matched argument is omitted and no config
value is available, the call fails with the standard missing-parameter error.
...
`packageName: 'core'` produces `CoreConfig`). For an SDK spanning multiple packages, shared keys
declared in several package blocks unify into one runtime config object — check it against the
intersection of the per-package interfaces:
