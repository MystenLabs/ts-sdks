...
title: Sui Clients
description: >-
  Choose between SuiGrpcClient and SuiGraphQLClient and understand their shared
  API
keywords:
  - Sui client
  - gRPC
...
answer: >-
  The Sui SDK offers three client types: SuiGrpcClient (recommended, fastest),
  SuiGraphQLClient (flexible queries), and SuiJsonRpcClient (legacy). All share
  the Core API interface for common operations.
goal:
  description: >-
    Reader can choose and configure the right Sui client (gRPC, GraphQL, or
    JSON-RPC) for their use case
  requires:
    - has_frontmatter:
        - title
        - description
