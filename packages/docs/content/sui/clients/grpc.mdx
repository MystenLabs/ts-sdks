...
and SDK integrations: it reads directly from a full node, has the most complete method surface, and
is the only client with real-time subscriptions.
...
| Service                        | Covers                                                     |
| ------------------------------ | ---------------------------------------------------------- |
| `ledgerService`                | Transactions, checkpoints, epochs, service info, list RPCs |
...
| `movePackageService`           | Packages, modules, functions, datatypes                    |
...
take a read mask naming the fields to return, and every call accepts an `RpcOptions` argument where
`abort` carries an `AbortSignal`. Reach for them when the top-level method cannot express what you
need — otherwise prefer the top-level method, which handles field selection and response mapping.
...
A `SCAN_LIMIT` end reason means the scan stopped early, not that results are exhausted. Reissue the
request with `options.after` set to the last watermark cursor to keep making progress.
