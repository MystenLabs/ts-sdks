...
- A datatype entry with no `fields` and no type arguments replaces the type's generated
...
- Every other entry replaces the type **wherever it is rendered**. That covers types with no
  ...
  (`pool::Pool<0x2::sui::SUI>`), and anything narrowed by `fields`.
- Replacement applies at **any depth**, because the type renderer consults overrides as it
  recurses. An override on `u64` also replaces the `u64` inside `vector<u64>` and `Option<u64>`. A
  whole-type match wins over its elements, so an entry for `vector<u64>` replaces the vector rather
  than its items.
- `fields` narrows which sites are replaced, written `module::TypeName.field` with `*` wildcards in
  the module, type, and field segments. Only sites whose type matches `type` are replaced. Enum
  variant fields match on both `field` and `variant.field` (for example, `Result.Err.code`).
- When several entries match one site, the most specific wins — the one naming more of the module,
  type, and field segments concretely — so a broad rule can be given narrower exceptions. Two
  entries matching equally specifically fail generation.
