...
Creates a wallet initializer for use with `createDAppKit({ walletInitializers: [...] })`:

```typescript
function devWalletClientInitializer(options?: DevWalletClientOptions): WalletInitializer;
```
