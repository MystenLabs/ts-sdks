	...
	packages: [
		{
			...
			bcsOverrides: [
				...
				// Replace a pure type at matching field sites only. `fields` accepts `*`
				// wildcards in the module, type, and field segments.
				{ type: 'u64', fields: 'order::Order.*_price', source: './src/bcs/units.ts#Price9' },
				{ type: 'u64', fields: 'pool_accounting::*.*', source: './src/bcs/units.ts#Usdc6' },
...
- Types that generated layouts serialize inline rather than by reference — primitives, `vector`,
  ...
  `0x2::object::UID` — can only be replaced at field sites, so they require a `fields` pattern.
- `fields` is written `module::TypeName.field` and only replaces fields whose declared Move type
  matches `type` exactly (an uninstantiated generic datatype matches every instantiation). Enum
...
- Two entries matching the same field fail generation; entries that never match anything produce a
  warning.
