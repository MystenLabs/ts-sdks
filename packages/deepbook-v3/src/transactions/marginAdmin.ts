...
export class MarginAdminContract {
	...
	registerDeepbookPool =
		(poolKey: string, poolConfig: TransactionArgument) =>
		(tx: Transaction): void => {
	 ...
	 */
	newPoolConfig =
		(poolKey: string, poolConfigParams: PoolConfigParams) =>
		(tx: Transaction): TransactionResult => {
			const pool = this.#config.getPool(poolKey);
			const baseCoin = this.#config.getCoin(pool.baseCoin);
			const quoteCoin = this.#config.getCoin(pool.quoteCoin);
	...
	newPoolConfigWithLeverage =
		(poolKey: string, leverage: number) =>
		(tx: Transaction): void => {
			const pool = this.#config.getPool(poolKey);
			const baseCoin = this.#config.getCoin(pool.baseCoin);
			const quoteCoin = this.#config.getCoin(pool.quoteCoin);
	 ...
	 */
	newCoinTypeData =
		(coinKey: string, maxConfBps: number, maxEwmaDifferenceBps: number) =>
		(tx: Transaction): TransactionResult => {
	 ...
	 * @returns A function that takes a Transaction object
	 */
	disableVersionPauseCap =
		(version: number, pauseCapId: string) =>
		(tx: Transaction): void => {
			tx.moveCall({
