...
```
`authenticatorSelection` can optionally be specified as follows:
- "cross-platform": For authenticators that work across multiple devices/platforms (like security keys, mobile authenticators)
- "platform": For device-specific authenticators (like Touch ID, Face ID, Windows Hello)
...
Passkey devices compliant with the WebAuthn standard support passkey wallets in Sui. 
For comprehensive device compatibility information, see [here](https://passkeys.dev/device-support/).