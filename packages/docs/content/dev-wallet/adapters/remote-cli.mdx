...
[standalone mode](/dev-wallet/standalone-mode). You can also use it programmatically:
...
await adapter.initialize(); // Fetches available accounts from CLI
```
...
| `name`            | `'Sui CLI'`                                  |
| `allowAutoSign`   | `false`                                      |
| `initialize()`    | Fetch available accounts from the CLI server |
| `getAccounts()`   | List accounts from CLI keystore              |
...
- `allowAutoSign` is always `false` — CLI signing should always require explicit approval
- Account creation, import, and removal are not supported — manage keys via `sui keytool`
