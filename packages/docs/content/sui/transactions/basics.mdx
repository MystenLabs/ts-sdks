...
Use `tx.balance()` and `balance::send_funds` to send SUI; this deposits
directly into the recipient's address balance:
...
<Callout type="warn">
	Always access elements by index or destructuring. Do not
	use the spread operator (`...result`) or pass results to `Array.from()`, which causes an
	infinite loop.
</Callout>
...
The `tx.add()` method accepts thunks, which are functions that receive the transaction and add commands to
...
See [SDK Building](/sui/sdk-building) for more information on building composable transaction libraries.
