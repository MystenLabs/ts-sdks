...
export function isTxContext(param: SuiClientTypes.OpenSignature): boolean {
	...
	return (
		normalizeSuiAddress(pkg) === normalizeSuiAddress('0x2') &&
		module === 'tx_context' &&
		name === 'TxContext'
	);
}
...
export function getPureBcsSchema(
	...
	switch (typeSignature.$kind) {
		...
		case 'datatype': {
			...
			if (normalizedPkg === normalizeSuiAddress(MOVE_STDLIB_ADDRESS)) {
			...
			if (normalizedPkg === normalizeSuiAddress(SUI_FRAMEWORK_ADDRESS)) {
