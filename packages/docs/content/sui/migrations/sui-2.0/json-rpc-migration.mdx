...
## Keeping the legacy client running

Migrations are rarely done in one step. `SuiJsonRpcClient` still ships in `@mysten/sui/jsonRpc` so
existing code keeps working while you move it, and because it implements the same
[Core API](/sui/clients/core), SDKs that accept `ClientWithCoreApi` support it throughout the
migration.
...
// Legacy top-level methods keep their JSON-RPC names and response shapes
const coins = await client.getCoins({ owner: '<OWNER_ADDRESS>' });

// client.core provides the same shapes as the gRPC and GraphQL clients
const { object } = await client.core.getObject({ objectId: '0x...' });
```

`getJsonRpcFullnodeUrl` accepts `'localnet'` (`http://127.0.0.1:9000`), `'devnet'`, `'testnet'`, and
`'mainnet'`.

### Legacy pagination

Legacy JSON-RPC methods return `data`, `nextCursor`, and `hasNextPage`, and some accept
`descendingOrder`. Core API methods use `cursor`/`hasNextPage` with the results under a named field
instead, so paginated call sites need updating as you migrate each one:
...
`SuiJsonRpcClient` creates a `JsonRpcHTTPTransport` by default. Construct one yourself to pass
headers or a custom `fetch`:
...
### Calling RPC methods directly

For methods with no client binding, `call` sends any JSON-RPC method the node exposes. This is a
useful stopgap when a legacy endpoint has no replacement yet:

```typescript
const committeeInfo = await client.call('suix_getCommitteeInfo', []);
```

See the [RPC documentation](https://docs.sui.io/references/sui-api) for the full method list.
