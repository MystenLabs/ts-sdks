...
2. Auto-approve check: adapter's `allowAutoSign` is evaluated first (adapters can opt out
   regardless of wallet policy), then the wallet-level `autoApprove` policy
