...
addresses to a runtime config object. Matchers are network-agnostic (`module::Type` for the
package's own types, `@pkg/name::module::Type` for dependency packages from the codegen config,
or framework `0x1`-`0x3`) and support type matchers, per-function matchers
...
parameter's normalized instantiation and call-site metadata. Each package emits a
