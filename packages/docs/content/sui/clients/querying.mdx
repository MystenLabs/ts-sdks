...
narrow as you need — a package, a module, a type name, or a full instantiation — so
`0x2::coin::Coin` matches every `Coin<T>` while `0x2::coin::Coin<0x2::sui::SUI>` matches only SUI
coins. The `type` accepts [MVR](#move-registry-names) names as well as fully qualified types.
