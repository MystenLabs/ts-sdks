...
App-bound allowances require a `SpendPermit<A>` from the app. Pass the permit and app type to
`tx.balance()` or `tx.coin()`:
...
A permit authorizes a spend but does not bind its amount or recipient. Apps that need to enforce
