...
`include: { transaction: true }` returns `SuiClientTypes.TransactionData`: the decoded ledger data
with `sender`, `gasData`, `expiration`, `version: 2`, `inputs`, `commands`, and `kind`. The same
model is used by reads, execution, simulation, lists, and streams. It represents both programmable
...
Both `ProgrammableTransaction` and `ProgrammableSystemTransaction` populate top-level `inputs` and
`commands`. Their `kind` payload mirrors those arrays using the same existing programmable
representation. Existing programmable-transaction consumers can continue reading the top-level
fields without changes.

Other variants include `ChangeEpoch`, `EndOfEpochTransaction`, `AuthenticatorStateUpdate`,
`RandomnessStateUpdate`, and the four consensus-commit prologue versions. Non-programmable
transactions have empty top-level `inputs` and `commands`; their actual payload is in `kind`. Empty
arrays describe the absence of a programmable body, not the absence of transaction activity. System
payloads use the decoded BCS shapes.

The `version: 2` field retains the SDK's existing serialization convention. It does not identify the
ledger transaction kind or make non-programmable transactions usable by the transaction builder.
Inspect `kind.$kind` to distinguish transaction kinds. The builder's `getData()`,
`SerializedTransactionDataV2`, and `parseTransactionBcs` utility retain their existing formats.

System transactions are returned as data, not skipped or replaced with `null`. Invalid transaction
bytes still produce an error. Requesting parsed transaction data alone does not expose the optional
`bcs` response field.
