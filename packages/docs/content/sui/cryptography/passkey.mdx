...
`PasskeyProvider` and provide the `rpName`, `rpId`, and additional options. The
`getPasskeyInstance` call initializes a new passkey wallet for the origin.
...
(for example, through a "Log in to existing passkey wallet" flow) to be able to use the passkey wallet again. If the user
...
signatures from past transaction history for the given address. This might be a preferred user experience
because the user is only prompted to sign one message, but the frontend needs to do extra work.
