...
While most apps should use the [`Connect Button`](/dapp-kit-next/web-components/connect-button)
which includes an integrated modal, the standalone modal is useful for advanced scenarios requiring
...
For standard wallet connection with a button, use the
[Connect Button](/dapp-kit-next/web-components/connect-button) component instead.
...
DApp Kit React bindings provide a
[React wrapper](../framework-guides/react/components/connect-modal) for the modal.
...
detected in the browser. For Next.js guide see [here](../framework-guides/next-js).
