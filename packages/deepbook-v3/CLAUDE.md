...
│   ├── account.ts         # `/account` subpath — the shared on-chain account primitive
│   ├── sessions.ts        # `/sessions` subpath — time-limited delegated authority
│   ├── predict/           # `/predict` subpath — DeepBook Predict (facade, quotes, receipts)
│   ├── bcs/integers.ts    # bigint BCS overrides used by the generated bindings
│   └── contracts/         # GENERATED by `pnpm codegen` — do not hand-edit
├── examples/              # Usage examples
├── PREDICT.md             # Long-form `/predict` guide, linked from the README
└── test/
    ├── unit/
    └── predict/
        └── testnet/       # Live-testnet suite; excluded from the default lane
```

Each subpath is a separate module graph and a separate `exports` entry. Adding one means updating
`package.json` `exports`, `tsdown.config.ts`, and `typedoc.json` together.
...
```bash
# Offline suite (every subpath, including /predict) — this is what CI runs
