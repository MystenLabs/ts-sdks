...
The `include` parameter controls what comes back with the result — see
[include options](/sui/clients/executing#include-options) for the full set.

Clients also expose `signAndExecuteTransaction`, which takes a signer instead of bytes and does both
steps in one call — useful when the signer is not a keypair, or in library code that holds a client.
See [Executing transactions](/sui/clients/executing) for the full set of client execution methods.
