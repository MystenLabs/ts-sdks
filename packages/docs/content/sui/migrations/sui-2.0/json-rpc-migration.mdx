...
| `getLatestCheckpointSequenceNumber` | `ledgerService.getCheckpoint` (omit `checkpointId` for the latest) |
| `getCurrentEpoch`                   | `ledgerService.getEpoch` (omit `epoch` for the current)            |
| `getLatestSuiSystemState`           | `ledgerService.getEpoch` (response includes `systemState`)         |
| `getCommitteeInfo`                  | `ledgerService.getEpoch` (response includes `committee`)           |
| `getProtocolConfig`                 | `ledgerService.getEpoch` (response includes `protocolConfig`)      |
...
// Get a specific checkpoint (omit checkpointId for the latest checkpoint)
const { response } = await client.ledgerService.getCheckpoint({
	checkpointId: { oneofKind: 'sequenceNumber', sequenceNumber: 12345n },
});

// Get the current epoch (omit `epoch` for the current one). The response also
// carries the system state, committee, and protocol config for that epoch.
