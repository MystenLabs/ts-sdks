...
A `PasskeyKeypair` is a `KeyPair` instance and a `PasskeyPublicKey` is a `PublicKey`. They can be 
used as part of Multisig address derivation and signing once initialized. 
