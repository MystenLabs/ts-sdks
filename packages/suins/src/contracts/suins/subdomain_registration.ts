...
export const SubDomainRegistration: MoveStruct<{
    "id": typeof bcs.Address;
    "nft": typeof suins_registration.SuinsRegistration;
}> = new MoveStruct({ name: `${$moduleName}::SubDomainRegistration`, fields: {
        id: bcs.Address,
...
export function nft(options: NftOptions): (tx: Transaction) => TransactionResult {
...
export interface NftMutOptions {
    package?: string;
    arguments: NftMutArguments | [
        name: RawTransactionArgument<string>
    ];
}
export function nftMut(options: NftMutOptions): (tx: Transaction) => TransactionResult {
    const packageAddress = options.package ?? '@suins/core';
    const argumentsTypes = [
        null
    ] satisfies (string | null)[];
