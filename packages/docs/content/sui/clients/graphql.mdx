...
malformed responses, and connection read failures throw a `SuiGraphQLSubscriptionError` or the
underlying transport error. Iteration ends when the server closes the response or sends an SSE
completion event.

Requests begin when iteration starts. Use `break` to leave the `for await...of` loop and close the
subscription, or call `controller.abort()` to stop while waiting for the next result. `subscribe`
does not reconnect automatically. Use [resumable ledger streams](./streaming.mdx) for historical
replay and automatic reconnect recovery.
