...
Generated types can build their own type tag strings with the `typeTag` method:

```typescript
import { Counter } from './contracts/counter/counter';
import { Balance } from './contracts/counter/deps/sui/balance';

// Types without phantom type parameters need no arguments
Counter.typeTag();
// '@local-pkg/counter::counter::Counter'

// Types with unfilled phantom parameters require typeArguments
Balance.typeTag({ typeArguments: ['0x2::sui::SUI'] });
// '0x2::balance::Balance<0x2::sui::SUI>'

// Override the package address or name
Counter.typeTag({ package: '0x123...' });
```

`typeArguments` is the full positional list of type arguments, in the order they are declared in
Move. Each argument can be a type tag string, another `typeTag()` result, or a BCS type (its name is
used). Arguments are validated against the type's declaration when the tag is built.

For packages configured with MVR names, the result contains those names. These are valid in
transaction `typeArguments` — they resolve automatically when the transaction is built — but for
query filters or comparisons against onchain data, use `resolveTypeTag` to get the fully resolved,
normalized form:
