...
fetch the gas price, estimate the gas budget, pick gas coins, and set an expiration. To build
without a client, provide that information yourself. See also the Sui documentation on
...
With `setGasPayment([])`, gas is paid from the gas owner's SUI address balance. If the transaction
has no owned object inputs, nothing ties it to a specific object version, so it also needs a
`ValidDuring` expiration for replay protection:
...
`{ Epoch: n }` expiration does not provide replay protection. Use `ValidDuring` unless the
transaction has an owned object input.
...
An owned object input already provides replay protection, so a transaction that has one can pay gas
from address balance without an expiration:
...
// Sets gas payment to [] because the transaction doesn't use tx.gas
const bytes = await tx.build({ assumeSufficientAddressBalances: true });
```

The SDK can't tell an owned object reference from an immutable one, so it lets this build. If every
object turns out to be immutable, execution rejects the transaction. Shared and party objects don't
count as replay protection.
...
For SUI, intents with the default `useGasCoin` still use the gas coin if anything else in the
transaction references `tx.gas`. Otherwise they withdraw from address balance. `useGasCoin: false`
always withdraws from address balance. Mixing the two in one transaction is still an error.

On a full build, the option also sets the gas payment to `[]` if none was set and the transaction
doesn't use `tx.gas`. Sender, gas price, gas budget, and replay protection still need to be
provided. Once set, the empty payment is part of the transaction, the same as calling
`setGasPayment([])`.
