...
const client = new SuiJsonRpcClient({
	...
	// Setting network on your client is optional for walrus to work correctly
	network: 'testnet',
}).$extend(walrus());
```
...
Walrus is designed to be handled some nodes being down, and the SDK will only throw errors when it
