...
import { OPERATION_TYPE_INTENT } from '@mysten/wallet-sdk';
...
export class DemoWallet implements Wallet {
	...
	connect: StandardConnectMethod = async (input) => {
		...
		return { accounts: selectedAccounts, supportedIntents: [OPERATION_TYPE_INTENT] };
	};
	...
	signTransaction: SuiSignTransactionMethod = async ({ transaction, account, chain }) => {
		...
		return new Promise((resolve, reject) => {
			this.#requestHandler({
				...
				resolve: async (txBytes) => {
					try {
						const keypair = this.#getKeypairForAccount(requestedAccount);
						resolve(await keypair.signTransaction(txBytes));
					} catch (error) {
			...
			this.#requestHandler({
				...
				resolve: async (txBytes) => {
					try {
						...
						const { transaction } = await suiClient.core.executeTransaction({
							transaction: txBytes,
						...
						resolve({
							...
							effects: toBase64(transaction.effects.bcs!),
						});
					} catch (error) {
						reject(error as Error);
					}
	...
	getSupportedIntents: SuiGetSupportedIntentsMethod = async () => {
		return {
			supportedIntents: [OPERATION_TYPE_INTENT],
		};
	};
