...
export const $wallets = atom<readonly WalletWithRequiredFeatures[]>([]);
...
onMount($wallets, () => {
	...
	const subscribeToWalletEvents = (wallet: WalletWithRequiredFeatures) => {
		const unsubscribeFromEvents = wallet.features[StandardEvents].on('change', () => {
			const suiWallets = get().filter(isSuiWallet);
			$wallets.set(suiWallets);
		});

		const walletId = getWalletUniqueIdentifier(wallet);
		unsubscribeCallbacksByWalletId.set(walletId, unsubscribeFromEvents);
	};
	...
	const unsubscribeFromUnregister = on('unregister', (...removedWallets) => {
		removedWallets.filter(isSuiWallet).forEach((wallet) => {
			const walletId = getWalletUniqueIdentifier(wallet);
			const unsubscribe = unsubscribeCallbacksByWalletId.get(walletId);
...
export function reg() {
	class Suiiii implements Wallet {
		#name: string;
		#chains: any[];
		#events: any;
		constructor(name: string) {
			this.#name = name;
			this.#chains = ['sui:mainnet'] as const;
		...
		get features() {
			return {
				'standard:connect': {
					version: '1.0.0',
					connect: () => {
						this.#chains = ['solana:abc'];
						this.#events.emit('change', { chains: ['solana:abc'] });
					},
				},
				'standard:events': {
					version: '1.0.0',
	...
	return getWallets().register(new Suiiii('ff'));
}
...
class EventEmitter {
	...
	off(event: string, listenerToRemove: any): void {
