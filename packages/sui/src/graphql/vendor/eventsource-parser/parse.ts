// Vendored from rexxars/eventsource-parser, commit 6519a0f70dfeb22f829e5c696da2cb4988d64c6a.
// Copyright (c) Espen Hovlandsdal. MIT license; see LICENSE and README.md.
/**
 * EventSource/Server-Sent Events parser
 * @see https://html.spec.whatwg.org/multipage/server-sent-events.html
 */
import { ParseError } from './errors.js';
import type { EventSourceParser, ParserConfig } from './types.js';
...
export function createParser(config: ParserConfig): EventSourceParser {
	...
	const { maxBufferSize, dispatchEmptyData, onComment, onError, onEvent, onId, onRetry } = config;
		...
		if (chunk.indexOf('\r') === -1) {
			...
			while (lfIndex !== -1) {
				...
				if (searchIndex === lfIndex) {
					...
					if (dataLines > 0 || (dispatchEmptyData && eventType !== undefined)) {
	...
	function parseLine(chunk: string, start: number, end: number) {
		...
		if (isEventPrefix(chunk, start, firstCharCode)) {
			// 'event:'.length === 6, 'event: '.length === 7
			eventType =
				chunk.slice(chunk.charCodeAt(start + 6) === SPACE ? start + 7 : start + 6, end) ||
				undefined;
			return;
		}
