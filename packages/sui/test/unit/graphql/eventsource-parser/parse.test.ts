// Adapted from rexxars/eventsource-parser, commit 6519a0f70dfeb22f829e5c696da2cb4988d64c6a.
// MIT license; see ../../../../src/graphql/vendor/eventsource-parser/LICENSE.
import { expect, test, vi } from 'vitest';

import { ParseError } from '../../../../src/graphql/vendor/eventsource-parser/errors.js';
import { createParser } from '../../../../src/graphql/vendor/eventsource-parser/parse.js';
import {
...
import { getParseResultMock } from './mock.js';
import { expectedMultiByteEvents } from './multibyte.js';
...
// Local GraphQL-over-SSE compatibility option.
test.each([false, true])('event-only dispatch is opt-in: %s', (dispatchEmptyData) => {
	const onEvent = vi.fn();
	const parser = createParser({ onEvent, dispatchEmptyData });
	for (const character of 'event: complete\r\n\r\n') parser.feed(character);
	expect(onEvent.mock.calls).toEqual(
		dispatchEmptyData ? [[{ event: 'complete', data: '', id: undefined }]] : [],
	);
});
test('event-only dispatch handles complete lines and ignores empty unnamed blocks', () => {
	const onEvent = vi.fn();
	const parser = createParser({ onEvent, dispatchEmptyData: true });
	parser.feed('event: complete\n\n\n: keepalive\n\n');
	expect(onEvent.mock.calls).toEqual([[{ event: 'complete', data: '', id: undefined }]]);
});
