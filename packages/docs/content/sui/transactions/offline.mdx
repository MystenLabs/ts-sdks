...
network. For offline building of backend services, air-gapped signing, or pre-built transactions, you
...
When your transaction only uses shared objects, party objects, and/or
address balance withdrawals, there are no owned object versions to look up. Use `tx.withdrawal()`
directly instead of `tx.coin()` or `coinWithBalance` because they require a client to resolve coin
...
- **No version lookup needed**:`initialSharedVersion` is stable amd set once when the object becomes
  a party object
- **Enable pipelining**: Submit multiple transactions on the same party object without
...
applies when using address balances for gas (`setGasPayment([])`) with only shared and party object
inputs:
