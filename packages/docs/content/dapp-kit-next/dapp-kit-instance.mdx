...
<Callout type="warn">
	If you use the React bindings, it's necessary that you [register global dAppKit
	type](./framework-guides/react/getting-started#create-a-dapp-kit-instance) for hooks to work
	properly.
</Callout>
...
- **storage** (optional) - Configures how the most recently connected to wallet account is stored
  (default: `localStorage`, set to s `null` to disable)
...
```ts
{
    ...
    signAndExecuteTransaction: (args: { transaction: Transaction | string; signal?: AbortSignal }) => Promise<SuiSignAndExecuteTransactionOutput>;
...
<Callout>
	If you use the React bindings, see [Getting Started in
	React](./framework-guides/react/getting-started) and
	[DAppKitProvider](./framework-guides/react/dapp-kit-provider).
</Callout>
