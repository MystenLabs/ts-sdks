...
title: Key pairs
description: >-
  Create Ed25519, Secp256k1, Secp256r1, and ML-DSA-65 keypairs and derive them from mnemonics and
  secret keys
keywords:
  - keypairs
  - Ed25519
...
| ML-DSA-65       | `MLDSA65Keypair`   | `@mysten/sui/keypairs/mldsa65`   |
...
ML-DSA-65 is the post-quantum scheme from FIPS 204. Its secret key is a 32-byte seed, so it imports
and exports like the other schemes, but its public keys (1952 bytes) and signatures (3309 bytes) are
much larger, and every signature is randomized. Deriving an ML-DSA-65 keypair from a mnemonic is not
supported yet, and the scheme is only accepted by networks where it has been enabled.

```typescript
import { MLDSA65Keypair } from '@mysten/sui/keypairs/mldsa65';

const keypair = MLDSA65Keypair.generate();
const restored = MLDSA65Keypair.fromSecretKey(keypair.getSecretKey());
```
