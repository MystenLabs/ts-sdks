...
1. **Custom URL Schemes** - Direct links using `slush://` or `suiwallet://` protocols (mobile app only)
...
Slush registers two custom URL schemes:

| Scheme         | Description                                    |
| -------------- | ---------------------------------------------- |
| `slush://`     | Primary scheme for the Slush wallet            |
| `suiwallet://` | Legacy scheme for backwards compatibility      |

Both schemes support identical paths and parameters. Use `slush://` for new integrations.
...
slush://claim?claimUrl=https://zksend.com/claim/abc123
```
...
<Callout type="info">
	Unlike custom URL schemes (`slush://`), `my.slush.app` links never fail - they always open
	either the native app or the web app, ensuring users can complete their intended action.
</Callout>
...
https://my.slush.app/claim/abc123xyz

# Legacy domain
https://suiwallet.com/ul/swap
```
...
These routes open the main wallet screens:

| Route      | Description                    | Example                  |
| ---------- | ------------------------------ | ------------------------ |
| `tokens`   | Home screen with token list    | `slush://tokens`         |
| `activity` | Transaction history            | `slush://activity`       |
| `nfts`     | NFT collection view            | `slush://nfts`           |
| `apps`     | dApp browser home              | `slush://apps`           |
| `settings` | Wallet settings                | `slush://settings`       |
| `earn`     | Yield and DeFi opportunities   | `slush://earn`           |
...
suiwallet://browse/https://bluefin.io
...
| `maxSlippage`  | number | No       | Maximum slippage percentage allowed              |
...
<Callout type="info">
	For zkSend and Getstashed claim links, the full HTTPS URL will also work directly:
	`https://zksend.com/claim/abc123xyz` will automatically open in Slush if installed.
</Callout>
...
Access account settings and management:
...
| `accounts/connect-ledger-device` | Connect a Ledger hardware wallet    |
...
slush://accounts
slush://accounts/add
slush://accounts/connect-ledger-device
```
...
| Route                             | Description                              |
| --------------------------------- | ---------------------------------------- |
| `onramp/fiat-input/:coinType`     | Enter fiat amount to spend               |
| `onramp/providers/:coinType/:amt` | Select provider for purchase             |

**Path Parameters:**

| Parameter  | Type   | Required | Description                    |
| ---------- | ------ | -------- | ------------------------------ |
| `coinType` | string | Yes      | Token type to purchase         |
| `amt`      | string | Yes      | Amount to purchase             |

**Examples:**

```
slush://onramp/fiat-input/0x2::sui::SUI
```
...
| Route                                   | Description                              |
| --------------------------------------- | ---------------------------------------- |
| `offramp/fiat-input/:coinType`          | Enter amount to sell                     |
| `offramp/providers/:coinType/:amount`   | Select provider for sale                 |
| `offramp/redirect/:provider/:coinType`  | Redirect to provider                     |

**Path Parameters:**

| Parameter  | Type   | Required | Description                    |
| ---------- | ------ | -------- | ------------------------------ |
| `coinType` | string | Yes      | Token type to sell             |
| `amount`   | string | Yes      | Amount to sell                 |
| `provider` | string | Yes      | Fiat provider name             |

**Examples:**

```
slush://offramp/fiat-input/0x2::sui::SUI
```
...
<Callout>
	Use `my.slush.app` links as your default choice. Only use custom URL schemes (`slush://`)
	when you specifically need to detect whether the mobile app is installed.
</Callout>
...
appropriate fallbacks. This is not needed when using `my.slush.app` links, which automatically
