...
For SUI, intents with the default `useGasCoin` still use the gas coin if anything else references
`tx.gas` when the intent is resolved. Otherwise they withdraw from address balance.
`useGasCoin: false` always withdraws from address balance. Mixing the two in one transaction is
still an error.

On a full build, the option also sets an unset gas payment to `[]`, but only when nothing else needs
a client, the transaction doesn't use `tx.gas`, and a `ValidDuring` expiration is already set.
Sender, gas price, and gas budget still need to be provided. Once set, the empty payment is part of
the transaction, the same as calling `setGasPayment([])`.
