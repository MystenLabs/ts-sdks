...
Two options change how the node runs the simulation. Both are available on `SuiGrpcClient` and
`SuiGraphQLClient`:
...
`checksEnabled: false` is what makes it possible to call a function that would normally fail
validation:
...
Left unset, `doGasSelection` follows the transaction: gas selection runs when the gas payment is
explicitly an empty list (`[]`), meaning gas comes from the sender's address balance. Transactions
with gas coins set are simulated as-is, and transactions with no gas payment are simulated with a
mocked gas coin.
