...
and the `source` module the replacement is imported from:
	...
	packages: [
		{
			...
			bcsOverrides: [
				...
				// A type with no generated declaration is replaced at its field sites. Without
				// `fields`, that means every field of that type in the package — here, parsing
				// every `u64` to a bigint instead of a decimal string.
				{ type: 'u64', source: './src/bcs/integers.ts#U64' },

				// `fields` narrows which sites are replaced, and accepts `*` wildcards in the
				// module, type, and field segments. The most specific entry matching a field
				// wins, so this carves an exception out of the blanket rule above.
				{ type: 'u64', fields: 'order::Order.*_price', source: './src/bcs/units.ts#Price9' },
				{ type: 'vector<u64>', fields: 'tree::Node.levels', source: './src/bcs/units.ts#Payouts' },
			],
		},
	],
};
...
export const Price9 = bcs.u64().transform({
...
`Order.parse(bytes).entry_price` is a `number` and `Order.serialize` accepts one.
...
- `type` uses the same package scoping as `configArguments` matchers (bare `module::Type` for the
  ...
  named-address labels from the package's summaries (for example, `fixed_math::i64::I64`) so
  entries can target dependency packages that are not codegen-run entries.
- A datatype entry without `fields` replaces the type's generated declaration: the generated module
  exports the custom type under the Move type's name, and reference sites are unchanged. Write the
  type without type arguments — the replacement applies to every instantiation, and for a generic
  type the source must export a function `(...typeParameters: BcsType<any>[]) => BcsType<any>`
  mirroring the generated call convention.
- Types that generated layouts serialize inline rather than by reference — primitives, `vector`,
  `0x1::string::String`, `0x1::ascii::String`, `0x1::option::Option`, `0x2::object::ID`, and
  `0x2::object::UID` — have no declaration to replace, so they are replaced at their field sites.
- `fields` is optional and narrows which field sites are replaced. Omit it to replace every
  matching field in the package. It is written `module::TypeName.field`, and only fields whose
  declared Move type matches `type` exactly are replaced (an uninstantiated generic datatype
  matches every instantiation). Enum variant fields match on both `field` and `variant.field` (for
  example, `Result.Err.code`). Supplying `fields` on a datatype that has a generated declaration
  replaces those field sites instead of the declaration.
- When several entries match one field, the most specific wins — the one naming more of the
  module, type, and field segments concretely — so a broad rule can be given narrower exceptions.
  Two entries matching equally specifically fail generation.
...
- An entry that matches nothing is not an error. A shared set of overrides can be applied to
  several packages even when only some of them have fields of a given type.
