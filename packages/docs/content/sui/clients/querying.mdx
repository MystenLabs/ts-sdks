...
List objects owned by an address, optionally filtered to an exact type. The `type` accepts
[MVR](#move-registry-names) names as well as fully qualified types.
...
<Callout type="warn">
	...
	UID fields as nested objects (`{ "id": { "id": "0x..." } }`), while gRPC and GraphQL flatten them
	(`{ "id": "0x..." }`). For consistent data across all clients, use `content` and parse BCS
	directly.
</Callout>
...
All three values are decimal strings, not numbers — use `BigInt` for arithmetic.
...
[include options](#include-options) as `getObject`. This one is only available on `client.core`.
...
Reading transactions back is a client concern; building, signing, and executing them is covered in
[Signing and execution](/sui/transactions/signing-and-execution). Alongside fetching a single
transaction, `listTransactions` and `listEvents` return filtered, cursor-paginated pages of history
— both take a `filter`, a `limit`, an `order`, and one cursor bound.

### `getTransaction`

Fetch one transaction by digest. The result is the same discriminated union that execution returns,
and it takes the same [include options](/sui/transactions/signing-and-execution#include-options).
...
A transaction that executed but aborted on-chain comes back as `FailedTransaction` rather than
throwing — see
[checking success or failure](/sui/transactions/signing-and-execution#checking-success-or-failure).

### `listTransactions`

Results use the same include options as `getTransaction`.
...
Each event carries its ledger position alongside the event data.
...
Both filters resolve [MVR](#move-registry-names) names automatically.

### Query cursors

`after` and `before` are exclusive ledger-position bounds. A query takes at most one of them, and
the bound implies the direction: `after` reads ascending, `before` reads descending. Each page
...
<Callout type="info">
	These queries scan the ledger, so a page can come back shorter than `limit` — even empty — while
	`hasNextPage` is still `true`. Drive the loop off `hasNextPage` rather than page length, and
	continue from `endCursor`: every request makes progress. Servers also cap page sizes (50 by
	default); over-large `limit` values are truncated on gRPC and rejected on GraphQL.
</Callout>

For filters beyond one predicate — combined or negated predicates, affected addresses and objects,
or checkpoint ranges — use the [raw gRPC list RPCs](/sui/clients/grpc#raw-list-rpcs) or a
[custom GraphQL query](/sui/clients/graphql#native-queries). To follow new activity as it happens,
see [gRPC subscriptions](/sui/clients/grpc#subscriptions) or
[GraphQL polling](/sui/clients/graphql#live-data).
...
human-readable alias such as `@mysten/sui` — which the client resolves for you. You can also resolve
them directly through `client.mvr`:
...
List methods that read collections — `listOwnedObjects`, `listCoins`, `listBalances`, and
`listDynamicFields` — share one pagination shape: pass `limit` and `cursor`, and read `cursor` and
`hasNextPage` back.
...
`listTransactions` and `listEvents` are ordered history queries rather than collection reads, so
they use the richer [query cursors](#query-cursors) described above instead of a single `cursor`.
...
Methods reject when a request fails. The one place errors are returned rather than thrown is
`getObjects`, which reports per-object failures in its result array so a single bad ID does not fail
the batch:
...
for (const object of objects) {
	if (object instanceof Error) {
		console.error('Error:', object.message);
	} else {
		console.log(object.objectId);
	}
}
...
Transaction results have their own convention, where a transaction that executed but failed on-chain
is not an error — see
[checking success or failure](/sui/transactions/signing-and-execution#checking-success-or-failure).
