...
An allowance lets a **spender** use a **funder's** address balance, subject to spending limits. The
funder creates it; the spender signs transactions that use it. The network must have allowance
support enabled (protocol v137 or later).

## Creating an allowance

This allowance permits up to 100 USDC of spending over 24 hours. `funder` is the signer creating the
allowance. Replace `0xUsdcPackage::usdc::USDC` with your network's USDC type; amounts use its six
decimal places.
...
[address balance](./coins-and-balances.mdx#depositing-into-address-balances) when the spender
withdraws.

### Limits and expiration

An allowance requires both a spending limit and an expiration, unless it has a recurring rate limit.
...
This skips the SDK's token type and app checks; incorrect references fail on-chain. Building may
still need a client to resolve object versions and gas.

## App-bound allowances

`tx.balance()` and `tx.coin()` support plain allowances. App-bound allowances also require a
`SpendPermit<A>` from the app. Use `tx.withdrawal()` with `app_balance_spend`:
