...
is spent. If prices increase by more than the buffer, the transaction is retried once with freshly
loaded prices, and a `StalePriceError` is thrown if it still fails.
