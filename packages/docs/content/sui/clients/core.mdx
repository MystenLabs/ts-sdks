...
keywords: [Core API, ClientWithCoreApi, transport-agnostic, Sui client, gRPC, GraphQL, JSON-RPC, TypeScript SDK]
---
...
The `objectBcs` option returns the full BCS-encoded object envelope, which is the struct content wrapped in
metadata (type, `hasPublicTransfer`, version, owner, previous transaction, and storage rebate). Most of
