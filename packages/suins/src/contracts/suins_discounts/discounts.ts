...
export const RegularDiscountsApp: MoveTuple<[ReturnType<typeof bcs.bool>]> = new MoveTuple({
	name: `${$moduleName}::RegularDiscountsApp`,
	fields: [bcs.bool()],
});
export const DiscountKey: MoveTuple<[ReturnType<typeof bcs.bool>]> = new MoveTuple({
 ...
 */
export function applyDayOneDiscount(
	options: ApplyDayOneDiscountOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@suins/discounts';
