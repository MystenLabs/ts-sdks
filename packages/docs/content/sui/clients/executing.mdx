...
`include: { transaction: true }` returns transaction data with `sender`, `gasData`, `expiration`,
`inputs`, `commands`, and `kind`. Use `kind.$kind` to identify the transaction type and access its
payload:
...
`ProgrammableTransaction` and `ProgrammableSystemTransaction` contain `inputs` and `commands`. Other
kinds, such as `Genesis`, `ChangeEpoch`, and `RandomnessStateUpdate`, have their own payloads.

The deprecated top-level `inputs` and `commands` mirror the programmable payload and are empty for
non-programmable transactions. Use the fields under `kind` in new code.
