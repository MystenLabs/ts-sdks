...
<AgentPrompt prompt="Help me start a TypeScript project that uses the Sui TypeScript SDK (@mysten/sui). Install it with npm i @mysten/sui, set up an ESM project with type module in package.json, then show me how to create a SuiClient against Testnet, generate a keypair, and read an account's balance." />
