...
- **Otherwise, coins are needed:** Fetches the sender's coin objects and address balance in parallel.
  ...
  For `tx.balance()` intents, the split results are converted to `Balance<T>` through
...
the gas price to zero with `tx.setGasPrice(0)`, but this causes the transaction to fail if it is
not actually gasless-eligible.

<Callout type="warning">
	When executing through a wallet (using dapp-kit), you might need to build the transaction first if
	the wallet itself is still using JSON-RPC to execute transactions.

    Call `tx.build({ client: grpcClient })` to use your grpc client to set the gas price
