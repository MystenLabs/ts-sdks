...
The `switchAccount` action changes the currently selected account to a different account from the connected wallet.

<Callout>
    Account switching is integrated into the [Connect Button](../web-components/connect-button) Web Component. When a wallet is connected with multiple accounts, the button provides an account selector.
</Callout>
