...
Skip the approval modal for automated testing, CI pipelines, or rapid prototyping. Auto-approval
policies control which signing requests are signed immediately without user interaction.
...
This is equivalent to the old `useUnsafeBurnerWallet` behavior — but with a real wallet-standard
implementation.
...
const wallet = new DevWallet({
	adapters: [
		new InMemorySignerAdapter(), // allowAutoSign: true → auto-approved
...
Without `autoConnect`, connect requests are queued for the user to select which accounts to expose
(similar to how MetaMask lets you choose accounts).
