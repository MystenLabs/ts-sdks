...
export function inactiveStake(
	options: InactiveStakeOptions,
...
export function createdProposal(
	options: CreatedProposalOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@deepbook/core';
	const argumentsTypes = [null] satisfies (string | null)[];
	const parameterNames = ['self'];
...
export function votedProposal(
	options: VotedProposalOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@deepbook/core';
	const argumentsTypes = [null] satisfies (string | null)[];
	const parameterNames = ['self'];
...
export function settledBalances(
	options: SettledBalancesOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@deepbook/core';
	const argumentsTypes = [null] satisfies (string | null)[];
	const parameterNames = ['self'];
