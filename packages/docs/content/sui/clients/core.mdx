...
Accept the client as a parameter rather than constructing one. Your caller has already configured a
network, endpoint, and transport, and creating a second client would bypass all of it — along with
the request cache the caller's client already holds.
...
Every client implements these methods on `client.core`. They are documented in full — with options,
include flags, and response shapes — in [Querying data](/sui/clients/querying).
...
### Methods only on `client.core`

Most of the contract is also mirrored as top-level methods on the concrete clients. These four are
not, so they are always reached through `client.core` — including from application code:

```typescript
const { object } = await client.core.getDynamicObjectField({
	parentId: '0x123...',
	name: { type: '0x2::object::ID', bcs: nameBytes },
});

const { systemState } = await client.core.getCurrentSystemState();
console.log(systemState.epoch, systemState.referenceGasPrice);

const { protocolConfig } = await client.core.getProtocolConfig();
console.log(protocolConfig.protocolVersion, protocolConfig.featureFlags);

const { chainIdentifier } = await client.core.getChainIdentifier();
```

`getChainIdentifier` returns the digest of the genesis checkpoint, base58-encoded — the value that
uniquely identifies a network. It never changes, so the client caches it after the first call.

`getReferenceGasPrice` returns `{ referenceGasPrice }` as a decimal string. You rarely need it
directly: the transaction builder sets the gas price for you.
...
| `listEvents` checkpoints    | `EventEntry.checkpoint` is `null` on JSON-RPC, which does not expose it                                                  |
| Generic `eventType` filters | A bare generic name matches any instantiation on gRPC and GraphQL, but nothing on JSON-RPC, which compares the full type |
...
| Subscriptions               | Real-time streams exist only on gRPC. See [subscriptions](/sui/clients/grpc#subscriptions)                               |
...
### Testing against every transport

Because these differences are real, the SDK's own parity tests run each case against all three
clients rather than trusting one. If your library depends on the contract, do the same — construct a
`SuiGrpcClient` and a `SuiGraphQLClient` in your test setup and run the same assertions through
both.
...
Every Core API method also accepts a `signal`, and library methods should forward the caller's
`AbortSignal` rather than swallowing it:
...
SDKs attach themselves to a client with `$extend`, which is what lets an application compose several
SDKs on one client instance:
...
See [Building SDKs](/sui/sdk-building) for how to author an extension, choose peer dependencies, and
structure your SDK's methods.
