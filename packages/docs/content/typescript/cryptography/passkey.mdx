...
- `cross-platform`: Use this for authenticators that work across devices, such as hardware security keys or mobile phones.
- `platform`: Use this for authenticators tied to a specific device, such as Touch ID, Face ID, or Windows Hello.
...
Sui supports passkey wallets on any device that complies with the WebAuthn standard. This includes most modern browsers and operating systems across desktop and mobile.
To check which platforms and authenticators are compatible, see the [Passkeys.dev device support list](https://passkeys.dev/device-support/).