...
**Coin objects** are individual onchain objects, each with its own ID, version, and balance. You
own specific coin objects and need to split, merge, and track them.

**Address balances** are an accumulator per address per coin type. There are no objects to manage because
...
- **Otherwise, coins are needed:** Fetches the sender's coin objects and address balance in parallel.
