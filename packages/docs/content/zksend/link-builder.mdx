...
if (result.$kind === 'FailedTransaction') {
	throw new Error(`Link regeneration failed: ${result.FailedTransaction.status.error}`);
}
```
...
// Check transaction status
if (result.$kind === 'FailedTransaction') {
	throw new Error(`Link creation failed: ${result.FailedTransaction.status.error}`);
}
```
