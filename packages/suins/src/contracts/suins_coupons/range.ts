...
export const Range: MoveStruct<{
    "vec": ReturnType<typeof bcs.vector<ReturnType<typeof bcs.u8>>>;
}> = new MoveStruct({ name: `${$moduleName}::Range`, fields: {
...
export interface IsInRangeOptions {
    package?: string;
    arguments: IsInRangeArguments | [
        range: TransactionArgument,
        number: RawTransactionArgument<number>
    ];
}
export function isInRange(options: IsInRangeOptions): (tx: Transaction) => TransactionResult {
    const packageAddress = options.package ?? '@suins/coupons';
    const argumentsTypes = [
        null,
        'u8'
...
export function to(options: ToOptions): (tx: Transaction) => TransactionResult {
    const packageAddress = options.package ?? '@suins/coupons';
    const argumentsTypes = [
        null
    ] satisfies (string | null)[];
