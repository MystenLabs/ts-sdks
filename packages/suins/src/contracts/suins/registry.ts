 ...
 */
export function addRecordIgnoringGracePeriod(
	options: AddRecordIgnoringGracePeriodOptions,
 ...
 * expired.
 */
export function burnRegistrationObject(
	options: BurnRegistrationObjectOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@suins/core';
...
export function wrapSubdomain(
	options: WrapSubdomainOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@suins/core';
	const argumentsTypes = [null, null, '0x2::clock::Clock'] satisfies (string | null)[];
 ...
 */
export function burnSubdomainObject(
	options: BurnSubdomainObjectOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@suins/core';
	const argumentsTypes = [null, null, '0x2::clock::Clock'] satisfies (string | null)[];
	const parameterNames = ['self', 'nft'];
 ...
 * hasn't expired.
 */
export function addLeafRecord(
	options: AddLeafRecordOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@suins/core';
 ...
 */
export function removeLeafRecord(
	options: RemoveLeafRecordOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@suins/core';
 ...
 */
export function assertNftIsAuthorized(
	options: AssertNftIsAuthorizedOptions,
): (tx: Transaction) => TransactionResult {
	const packageAddress = options.package ?? '@suins/core';
	const argumentsTypes = [null, null, '0x2::clock::Clock'] satisfies (string | null)[];
	const parameterNames = ['self', 'nft'];
