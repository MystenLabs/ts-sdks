...
**Breaking: removed `ACCUMULATOR_ROOT_ID`.** The generated move-call layer injects the well-known `AccumulatorRoot` singleton itself, as it does the `Clock`, so no caller needs the id to build a PTB against these bindings. Callers who passed it explicitly can drop the argument; anyone still needing the literal can use `0xacc` directly.
