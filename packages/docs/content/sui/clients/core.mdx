...
Every method here is also available as a top-level method on the concrete clients, so the choice
between `client.core.getObject()` and `client.getObject()` is about portability rather than reach.
