...
keywords: [transaction executor, SerialTransactionExecutor, ParallelTransactionExecutor, Sui, gas coins, execution, queuing, TypeScript SDK]
---

The TypeScript SDK ships two Transaction executor classes that simplify the processes of efficiently
...
- `sourceCoins`: An array of coins to use to create the gas pool, defaults to using all coins owned
  by the signer.
- `epochBoundaryWindow` Time to wait before or after the expected epoch boundary before re-fetching the
