...
generated parameter of their own package produce a warning. Note that the generated
`config-arguments.ts` interface covers the package's resolvable keys even when only some of its
functions consume them; the per-function `config` slices are the authoritative types for calls.
