...
Build custom gRPC-web transports from this class to retain these error-handling fixes.

`SuiGrpcClient` adds an `x-sui-client-protocol-version` request header with the highest Sui protocol
version whose types the SDK can decode. This applies to all transports, including native gRPC, and
to unary and streaming requests. Explicit values in the transport's `meta` options or an individual
service call's `meta` override the default. The client does not mutate a supplied transport or
