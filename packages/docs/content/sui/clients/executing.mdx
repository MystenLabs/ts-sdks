...
The signer can be any `Signer` — a keypair, or a KMS or Ledger signer. Pass `additionalSignatures`
for transactions needing more than one, such as a sponsored transaction the sponsor has already
signed.
...
executed but aborted on-chain comes back as `FailedTransaction`.
...
A resolved promise means the transaction was executed, which is not the same as it having succeeded
— see
...
`include` options. Everything is off by default, and a field that was not requested is typed
`undefined`:
...
It also accepts a `digest` instead of a result, along with `timeout` and `pollInterval`.
...
- [Signing and execution](/sui/transactions/signing-and-execution) for keypairs, wallets, gas, and
  sponsored transactions
- [Querying data](/sui/clients/querying) for reading transactions back
