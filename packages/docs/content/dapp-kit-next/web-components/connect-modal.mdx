...
While most apps should use the [`Connect Button`](/dapp-kit-next/web-components/connect-button) which includes an
integrated modal, the standalone modal is useful for advanced scenarios requiring custom trigger mechanisms.
...
For standard wallet connection with a button, use the [Connect Button](/dapp-kit-next/web-components/connect-button) component instead.
...
<!-- Modal component -->
<mysten-dapp-kit-connect-modal id="wallet-modal"></mysten-dapp-kit-connect-modal>
...
DApp Kit React bindings provide a [React wrapper](../framework-guides/react/components/connect-modal) for the modal.
...
The `<mysten-dapp-kit-connect-modal>` Web Component can be only client-side rendered, as wallets are detected in the browser. For Next.js guide see [here](../framework-guides/next-js).
