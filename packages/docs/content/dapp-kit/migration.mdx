...
fetching solution. The new client uses gRPC for communication with Sui nodes. Use standard React
patterns or integrate with TanStack Query if desired.
...
+ 	const [data, setData] = useState<{ objectId?: string; contents?: { value?: Uint8Array } } | null>(null);
...
+ 			try {
+ 				// Use the gRPC ledgerService directly
