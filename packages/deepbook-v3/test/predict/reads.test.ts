...
import { PredictMoveError } from '../../src/predict/errors.js';
import { accountBalance, hasPosition } from '../../src/predict/reads/balances.js';
...
import {
	...
	referenceTick,
	settlementPrice,
} from '../../src/predict/reads/markets.js';
import {
	parseOptionalId,
	parseOptionalU64,
	parseU64LE,
	parseVectorOfIds,
} from '../../src/predict/reads/parse.js';
import { poolStats } from '../../src/predict/reads/pool.js';
...
describe('parsers (pure, round-trip via bcs)', () => {
	...
	test('parseVectorOfIds round-trips a bcs vector<address>', () => {
		...
		expect(parseVectorOfIds(bcs.vector(bcs.Address).serialize([ADDR_A, ADDR_B]).toBytes())).toEqual(
			[ADDR_A, ADDR_B],
		);
	});
...
describe('inspectReturns (the network seam)', () => {
	test('sets sender, checksEnabled:false, include.commandResults; returns per-command bcs', async () => {
		...
		expect(
			(captured.opts?.include as { commandResults?: boolean } | undefined)?.commandResults,
		).toBe(true);
