...
The simplest adapter — generates Ed25519 keypairs held in memory. Keys are lost when the page
reloads. Perfect for quick prototyping, unit tests, and documentation demos.
...
<Callout type="info">
	Since keys exist only in memory, `InMemorySignerAdapter` is the only adapter that works
	identically in both browser and Node.js environments.
</Callout>
