...
- 504644c: Move the margin surface onto Pyth's upgraded Core and drop the legacy Pyth surface.
  ...
  accept — their signatures are frozen by the `compatible` upgrade policy. `deepbook_margin`
  ...
  **Requires the upgraded margin package on the target network, enabled.** The upgraded modules do
  not exist in earlier `deepbook_margin` publications, so this release must not be used against a
  ...
  **`liquidateBase` / `liquidateQuote` have no mainnet target yet.** They call
  ...
  none are intended to fall back to a DeepBook-operated proxy, which is not deployed yet, so that
  path currently throws a `ConfigurationError` naming the field to set.

  Package ids move to `deepbook_margin` v16 on testnet and v7 on mainnet, and `margin_liquidation`
