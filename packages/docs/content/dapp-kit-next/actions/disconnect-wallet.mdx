...
The `disconnectWallet` action handles errors gracefully:

```typescript
import { WalletNotConnectedError } from '@mysten/dapp-kit-core';
...
try {
  ...
  if (error instanceof WalletNotConnectedError) {
    console.log('No wallet was connected');
  }
}
```
...
The disconnect functionality is integrated into the `<mysten-dapp-kit-connect-button>` component. When a wallet is connected, the button displays the connected account and provides a disconnect option:
