...
replacement is imported from, optionally narrowed to specific `fields`. A datatype for which codegen
emits a declaration has that declaration replaced, so every use picks it up; everything else is
