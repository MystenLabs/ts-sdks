...
<Callout type="warn">
	**Incubation package.** This package is still experimental and may change without notice. Pin
...
draws from the sponsor's on-chain SUI balance rather than from specific gas coins. This means the
sponsor can sign and execute in parallel:

Address-balance gas doesn't lock specific coin objects, so concurrent sponsored transactions don't
contend for gas coins. A _sender_ that reuses its own versioned objects — coins — across rapid
...
`simulationSucceeds()` avoids paying for a transaction that aborts; and `boundedExpiration()`
requires a bounded expiration for address-balance gas.
