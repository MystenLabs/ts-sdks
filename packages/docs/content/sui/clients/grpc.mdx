...
| Native gRPC | Server-side (Node.js, Bun) | HTTP/2 | Server-side apps, backend services, CLI tools |

The native gRPC transport may offer better performance for high-throughput server-side applications
since it uses HTTP/2 directly without the gRPC-web translation layer.
