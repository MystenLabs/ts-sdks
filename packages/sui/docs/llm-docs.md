...
Every @mysten/_ package ships LLM documentation in its `docs/` directory. When working with these
packages, find the relevant docs by looking for `docs/llms-index.md` files inside
`node_modules/@mysten/_/`. Read the index first to find the page you need, then read that page for
details.
```
