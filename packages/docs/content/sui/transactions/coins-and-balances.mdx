...
the gas price will be set when the transaction is built.
...
<Callout type="warning">
	When executing through a wallet (using dapp-kit), you may need to build the transaction first if
	the wallet itself is still using JSON-RPC to execute transactions.
