...
The Sui dApp Kit Next uses [nanostores](https://github.com/nanostores/nanostores) for state management, providing a lightweight, framework-agnostic solution that works across React, Vue, Vanilla JavaScript, and other frameworks.
...
- **`$currentClient`** - SuiClient instance for the current network
...
wallets.forEach(wallet => {
  ...
  console.log('Installed:', wallet.isInstalled);
});
```

## Framework Integration

In vanilla JavaScript, subscribe directly to stores:
...
```tsx
import { useCurrentAccount, useCurrentNetwork, useWallets } from '@mysten/dapp-kit-react';
...
function MyComponent() {
  ...
  // These hooks automatically subscribe and update
  return <div>{currentAccount?.address}</div>;
}
```
