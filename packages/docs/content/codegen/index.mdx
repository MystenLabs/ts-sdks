...
Configure package overrides when creating your network config:

```typescript
import { createNetworkConfig } from '@mysten/dapp-kit-core';
import { SuiClient } from '@mysten/sui/client';
...
const { networkConfig } = createNetworkConfig({
  testnet: {
    url: 'https://fullnode.testnet.sui.io:443',
    variables: {
      counterPackageId: '0xYOUR_PACKAGE_ID',
    },
  },
});
  ...
  createClient={(network, config) => {
    return new SuiClient({
      ...
      mvr: {
        overrides: {
          packages: {
            '@local-pkg/counter': config.variables.counterPackageId,
          },
        },
      },
    });
...
- [create-dapp](/dapp-kit/getting-started/create-dapp) - Bootstrap a working dApp with codegen already configured
