...
ID. This is done using the `operationType` function, which adds a special intent to the transaction
that tells wallets which operation from the policy this transaction represents.
...
tx.add(operationType('gameplay')); // Tag with operation ID from policy
tx.add(contract.doAction(params)); // Add your transaction commands
```
