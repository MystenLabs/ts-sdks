...
+ });
=======
## zkSend SDK Migration

The `@mysten/zksend` SDK has been updated to use a client extension pattern with the new core API,
enabling compatibility with gRPC, GraphQL, and JSON RPC transports.

### Breaking Changes

- **Client extension pattern**: The zkSend SDK now uses a client extension pattern
  (`client.$extend(zksend())`).
...
Update your code to use the client extension pattern:
...
-   network: 'testnet',
- });
+ // Loading a link by address or keypair
...
The `@mysten/kiosk` package has been refactored to use the new core client API and generated
contract bindings.
...
`TransferPolicyTransaction` builder classes. These classes provide better autocomplete, handle
personal kiosks automatically, and ensure proper transaction finalization.

**Removed kiosk functions:**
...
**Removed transfer policy functions:**
...
**Removed personal kiosk functions:**
...
**Removed rule attachment functions:**
...
The Kiosk SDK now uses the core client API. Update your `KioskClient` initialization:
