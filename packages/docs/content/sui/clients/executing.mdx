...
with `sender`, `gasData`, `expiration`, `inputs`, `commands`, and `kind`. The same model is used by
reads, execution, simulation, lists, and streams. It represents both programmable and system
transactions.
