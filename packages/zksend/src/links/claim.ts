...
export class ZkSendLink {
	...
	async loadAssets(
		options: {
			transaction?: SuiClientTypes.TransactionResponse;
			loadAssets?: boolean;
	...
	async claimAssets(
		...
		if (!result.transaction.effects.status.success) {
			throw new Error(
				`Claim transaction failed: ${result.transaction.effects.status.error ?? 'Unknown error'}`,
			);
		}
	...
	async #loadBag({
		...
		const objectsResponse = await this.#client.core.listObjects({
			objectIds: itemIds,
			include: {
				content: true,
			},
