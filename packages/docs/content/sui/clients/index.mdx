...
The query methods return filtered, cursor-paginated history and behave identically on every
transport. See [Core API query methods](/sui/clients/core#query-methods) for filters and pagination.
...
Real-time streams are a gRPC-only capability: `SuiGrpcClient` can
[subscribe](/sui/clients/grpc#subscriptions) to checkpoints, transactions, and events, while GraphQL
consumers [poll with cursors](/sui/clients/graphql#subscriptions-and-live-data).
